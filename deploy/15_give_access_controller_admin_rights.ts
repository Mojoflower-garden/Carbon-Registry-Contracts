...
const main: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  const devProjectContracts = [
    ...
    '0x826b76bA7B9e9e1f19407FBA3d3011E37536dB58', // The CarbonregistryContract
  ];
  ...
  for (let i = 0; i < contractsToGrant.length; i++) {
    ...
    console.log('HASROLE:', hasRole, accessController);
    if (!hasRole) {
  ...
  console.log('Checking if roles are granted');
  // Check if has role for all contracts
  for (let i = 0; i < contractsToGrant.length; i++) {
    const accessControllerContract = await ethers.getContractAt(
      'AccessControlUpgradeable',
      contractsToGrant[i],
...
export default main;
main.tags = ['AccessControlControlerRights', '15'];
